...
def client_commande_add():
    ...
    for item in panier:
        ...
        sql = '''insert into ligne_de_commande(id_LigneMeuble, id_LigneCmd, prix_unit, quantite)
              values(%s,%s,%s,%s);'''
        mycursor.execute(sql, (item['id_PanierMeuble'], commande_last_id['last_insert_id'], prix['prix'],
...
def client_commande_show():
    ...
    if id_cmd is None:
        ...
        sql = '''select M.nom, L.quantite, M.prix, sum(L.quantite * M.prix) as prix_ligne
                from commande C
